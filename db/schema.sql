...
create table if not exists events (
        id integer primary key autoincrement,
        "start_on[value]" integer not null, -- months since 1 BCE
        "start_on[precision]" integer not null, -- 0 (exact), 1 (after), 2 (before), 3 (circa, about)
        "end_on[value]" integer, -- months since 1 BCE
        "end_on[precision]" integer, -- 0 (exact), 1 (after), 2 (before), 3 (circa, about)
        layer_id integer not null references layers(id),
        title varying char not null,
        created_at timestamp not null,
        updated_at timestamp not null
);
create index if not exists events_start_on_and_precision on events("start_on[value]", "start_on[precision]");
create index if not exists events_end_on_and_precision on events("end_on[value]", "end_on[precision]");
